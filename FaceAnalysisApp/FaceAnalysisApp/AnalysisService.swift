...
final class AnalysisService: AnalysisServing {
    ...
    func analyze(image: UIImage) -> AnyPublisher<AnalysisResult, AnalysisError> {
        Future { [weak self] promise in
            ...
            processingQueue.async {
                ...
                enableCPUInferenceIfNeeded(faceRectanglesRequest)
                ...
                for revision in revisions {
                    ...
                    enableCPUInferenceIfNeeded(request)
    ...
    private func classifyFace(dimensions: FaceDimensions) -> String {
        ...
        let jawTaper = jaw / max(cheekbone, 1.0)
        let foreheadDominance = forehead / max(cheekbone, 1.0)
        let normalizedJawAngle = jawAngle * 180.0 / .pi

        if lengthRatio < 1.05 {
            if normalizedJawAngle < 98 {
                return "square"
            }
            if foreheadToJaw > 1.12 && cheekToJaw > 1.15 {
                return "heart"
            }
            if abs(foreheadDominance - 1.0) < 0.05 && jawTaper > 0.95 {
                return "round"
            }
            return "round"
        }
        ...
        if lengthRatio >= 1.5 {
            if foreheadToJaw > 1.02 && cheekToJaw > 1.05 && jawTaper < 0.95 {
                return "heart"
            }
            return "oblong"
        }
        ...
        if lengthRatio >= 1.3 {
            if cheekToJaw > 1.15 && foreheadToJaw < 0.95 {
                return "diamond"
            }
            if foreheadToJaw > 1.02 && cheekToJaw > 1.05 && jawTaper < 0.96 {
                return "heart"
            }
        }

        if foreheadToJaw > 1.03 && cheekToJaw > 1.06 && jawTaper < 0.97 {
            return "heart"
        }

        if lengthRatio > 1.2 {
            if foreheadToJaw > 1.01 && cheekToJaw > 1.04 && jawTaper < 0.98 {
                return "heart"
            }
            return "oblong"
        }
    ...
    private func buildInsights(
        from points: FaceLandmarkPoints,
        boundingBox: CGRect,
        dimensions: FaceDimensions,
        skinTone: (tone: String, undertone: String, rgb: [Int]),
        faceShape: String
    ) -> FeatureInsights {
        let nose = points.noseTip ?? CGPoint(x: boundingBox.midX, y: boundingBox.midY)
        let leftCheekPoint = points.leftCheek ?? CGPoint(x: boundingBox.minX, y: boundingBox.midY)
        let rightCheekPoint = points.rightCheek ?? CGPoint(x: boundingBox.maxX, y: boundingBox.midY)

        let leftDistance = distance(leftCheekPoint, nose)
        let rightDistance = distance(rightCheekPoint, nose)
        let maxDistance = max(max(leftDistance, rightDistance), 1.0)
        let balanceDelta = abs(leftDistance - rightDistance) / maxDistance
        let symmetryScore = max(0, 1 - min(1, balanceDelta))

        let symmetryDescription: String
        switch symmetryScore {
        case 0.9...:
            symmetryDescription = "Highly balanced cheeks with even spacing from the nose."
        case 0.75..<0.9:
            symmetryDescription = "Soft asymmetry adds character—light contour evens things out."
        default:
            symmetryDescription = "Cheek width varies; targeted contour helps create balance."
        }

        let leftEye = points.leftEyeCenter ?? CGPoint(x: boundingBox.minX, y: nose.y)
        let rightEye = points.rightEyeCenter ?? CGPoint(x: boundingBox.maxX, y: nose.y)
        let eyeDeltaY = abs(leftEye.y - rightEye.y)
        let eyeAlignmentDegrees = atan2(Double(eyeDeltaY), Double(max(boundingBox.width, 1))) * 180.0 / .pi

        let guidance: String
        if eyeAlignmentDegrees < 1.0 {
            guidance = "Mirror highlight along both brow bones to emphasize your natural symmetry."
        } else if eyeAlignmentDegrees < 2.5 {
            guidance = "Add a touch more highlight on the slightly lower brow to visually lift it."
        } else {
            guidance = "Blend contour upward on the lower side and keep shimmer focused on the higher brow."
        }

        let browReferenceY = boundingBox.midY
        let browDelta = abs((points.leftBrow?.y ?? browReferenceY) - (points.rightBrow?.y ?? browReferenceY))
        let browBalanceScore = max(0.0, 1.0 - min(1.0, Double(browDelta / max(boundingBox.height * 0.25, 1))))

        let jawRatio = dimensions.cheekboneWidth / max(dimensions.jawWidth, 1.0)
        let jawDefinitionScore = max(0.0, 1.0 - min(1.0, abs(jawRatio - 1.0)))

        let toneSummary = buildToneSummary(for: skinTone)
        let featureRatios = buildFeatureRatios(for: dimensions)

        return FeatureInsights(
            symmetryScore: symmetryScore,
            symmetryDescription: symmetryDescription,
            eyeAlignmentDifference: eyeAlignmentDegrees,
            guidance: guidance,
            browBalanceScore: browBalanceScore,
            jawDefinitionScore: jawDefinitionScore,
            featureRatios: featureRatios,
            toneSummary: toneSummary,
            faceShape: faceShape
        )
    }

    private func buildToneSummary(for skinTone: (tone: String, undertone: String, rgb: [Int])) -> FeatureInsights.ToneSummary {
        ...
        let undertoneKeywords: [String: [String]] = [
            "warm": ["Golden", "Sunlit", "Honey"],
            "cool": ["Rosy", "Berry", "Icy"],
            "neutral": ["Balanced", "Adaptive", "Versatile"]
        ]

        let finishTips: [String: [String]] = [
            "warm": [
            ...
            "neutral": [
                "Blend warm and cool blushes to shift the look effortlessly.",
                "Try satin finishes for contour to stay flexible."
            ]
        ]

        let keywords = undertoneKeywords[skinTone.undertone, default: undertoneKeywords["neutral"] ?? []]
        let tips = finishTips[skinTone.undertone, default: finishTips["neutral"] ?? []]

        return FeatureInsights.ToneSummary(hex: hex, keywords: keywords, finishTips: tips)
    }

    private func buildFeatureRatios(for dimensions: FaceDimensions) -> [FeatureInsights.FeatureRatio] {
        var ratios: [FeatureInsights.FeatureRatio] = []

        func addRatio(
            name: String,
            value: Double,
            ideal: Double,
            descriptor: (Double) -> String
        ) {
            let delta = value - ideal
            ratios.append(
                FeatureInsights.FeatureRatio(
                    name: name,
                    value: value,
                    ideal: ideal,
                    delta: delta,
                    message: descriptor(delta)
                )
            )
        }

        let lengthRatio = dimensions.faceLength / max(dimensions.cheekboneWidth, 1.0)
        addRatio(
            name: "Face length vs. width",
            value: lengthRatio,
            ideal: 1.33,
            descriptor: { delta in
                if delta > 0.15 { return "Elongated" }
                if abs(delta) < 0.05 { return "Balanced" }
                return "Softly wider"
            }
        )

        let foreheadToJaw = dimensions.foreheadWidth / max(dimensions.jawWidth, 1.0)
        addRatio(
            name: "Forehead vs. jaw width",
            value: foreheadToJaw,
            ideal: 1.0,
            descriptor: { delta in
                if delta > 0.1 { return "Stronger forehead" }
                if abs(delta) < 0.05 { return "Even width" }
                return "Defined jaw"
            }
        )

        let cheekToJaw = dimensions.cheekboneWidth / max(dimensions.jawWidth, 1.0)
        addRatio(
            name: "Cheekbone vs. jaw width",
            value: cheekToJaw,
            ideal: 1.05,
            descriptor: { delta in
                if delta > 0.1 { return "Pronounced cheekbones" }
                if abs(delta) < 0.05 { return "Balanced" }
                return "Softer cheeks"
            }
        )
...
private func enableCPUInferenceIfNeeded(_ request: VNRequest) {
    if #available(iOS 17, *) {
...
private struct FaceLandmarkPoints {
    ...
    init(observation: VNFaceObservation, boundingBox: CGRect) {
        ...
        let leftEyePoints = convert(region: landmarks?.leftEye)
        let rightEyePoints = convert(region: landmarks?.rightEye)
        let nosePoints = convert(region: landmarks?.nose)
        ...
        leftEyeCenter = centroid(of: leftEyePoints)
        rightEyeCenter = centroid(of: rightEyePoints)
        noseTip = nosePoints.last ?? centroid(of: nosePoints)
    }
}
